...
function select(target, timer1, timer2){
    if(!choice1){
        choice1 = target;
        target.text = "First choice";
        target.color = turq;
        setTimeout(() => revealer(), timer1);
        play(timer2)
...
function simulatorRender(){
    ...
    document.querySelector('#stats').innerHTML = (`<h2 class="text-center text-dark my-3">Stats</h2><h5 class="text-center border rounded shadow p-3 mb-5"><b>Stay:</b> ${stay}<br><b>Switch:</b> ${change}<br><b>Total:</b> ${total}<br><br><button id="auto" class="btn btn-sm btn-dark text-warning m-2"><b>AUTO-PLAY</b></button><button id="auto-1000" class="btn btn-sm btn-dark text-danger m-2"><b>WARP-SPEED</b></button></h5>`)
    
    document.querySelector('#door-1').addEventListener('click', function (){
        select(door1, 500, 1500);
    });
    document.querySelector('#door-2').addEventListener('click', function(){
        select(door2, 500, 1500);
    });
    document.querySelector('#door-3').addEventListener('click', function(){
        select(door3, 500, 1500);
    });
